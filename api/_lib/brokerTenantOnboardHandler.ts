...
const ONBOARD_TOKEN_PREFIX = "broker_tenant_onboard_";
const INVITES_KEY = "broker_tenant_onboarding_invites";
...
function tokenDataKey(token: string): string {
  return `${ONBOARD_TOKEN_PREFIX}${token}`;
}
...
async function loadOnboardStore(): Promise<OnboardStore> {
  if (vercelDb.usePostgres()) {
    return {
      findEntryByDataKey: async (dataKey) => {
...
async function markInviteSubmitted(
  ...
  try {
    ...
    await store.setAccountDataKey(brokerPhone, "broker", INVITES_KEY, JSON.stringify(updated));
  } catch {
